...
System.out.println(b);
	...
	if(a.equals("color"))
	{
		...
		String sql1="Select numb,col from color1 where NUMB=(select  min (numb) from color1)";
	...
	{
		String q="Create view type1 as (Select count(type)as numb ,type as col from Booking_Details,Car_type,User1 where Booking_Details.Car_ID=Car_type.Car_Id and User1.SSN=Booking_Details.Cust_ID and User1.Gender='"+b+"' group by type)";
		
		String sql1="Select numb,col from type1 where NUMB=(select min (numb) from type1)";
		Statement stmt=con.createStatement();
		Statement stmtt=con.createStatement();
		stmt.executeUpdate(q);
		ResultSet rs1=stmtt.executeQuery(sql1);
	...
	{
		String q="Create view model1 as (Select count(car_name)as numb ,car_name as col from Booking_Details,Car_type,User1 where Booking_Details.Car_ID=Car_type.Car_Id and User1.SSN=Booking_Details.Cust_ID and User1.Gender='"+b+"' group by car_name)";
		
		String sql1="Select numb,col from model1 where NUMB=(select min (numb) from model1)";
		Statement stmt=con.createStatement();
		Statement stmtt=con.createStatement();
		stmt.executeUpdate(q);
		ResultSet rs1=stmtt.executeQuery(sql1);
